...
const ChartsScreen = ({ route, navigation }) => {
  ...
  // Update context insights when selected metric changes
  useEffect(() => {
    if (healthData.length > 0) {
      const insights = analyzeContextImpact(healthData, selectedMetric);
      setContextInsights(insights || []);
    }
  }, [selectedMetric, healthData]);
  ...
  const loadData = async () => {
    ...
    const insights = analyzeContextImpact(data, selectedMetric);
    setContextInsights(insights || []);
      ...
      >
        <View style={styles.header}>
          <View style={styles.headerContent}>
            <Text style={[styles.title, { color: theme.colors.text.light }]}>Health Insights</Text>
            <TouchableOpacity 
        ...
        {healthData.length === 0 ? (
          ...
          <View style={styles.chartsContainer}>
            ...
            {/* Add the Context Insights component if there are insights available */}
