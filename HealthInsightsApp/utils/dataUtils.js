...
// This function analyzes how context flags may affect metrics
export const analyzeContextImpact = (data, metric) => {
if (!data || data.length < 5) {
  return null; // Not enough data for meaningful analysis
}

// Group data by whether specific context flags were present
const contextImpacts = {};
const contextOptions = ['travel', 'illness', 'stress', 'exercise', 'relaxation'];

// Initialize context impacts
contextOptions.forEach(context => {
  contextImpacts[context] = {
    withContext: [],
    withoutContext: []
  };
});
...
// Populate data
data.forEach(entry => {
  if (!entry[metric] && entry[metric] !== 0) return;
  
  const entryContexts = entry.contextFlags || [];
  
  contextOptions.forEach(context => {
    if (entryContexts.includes(context)) {
      contextImpacts[context].withContext.push(entry[metric]);
    } else {
      contextImpacts[context].withoutContext.push(entry[metric]);
    }
  });
});
...
contextOptions.forEach(context => {
  const withContextData = contextImpacts[context].withContext;
  const withoutContextData = contextImpacts[context].withoutContext;
  
  if (withContextData.length >= 2 && withoutContextData.length >= 2) {
    const withAvg = withContextData.reduce((sum, val) => sum + val, 0) / withContextData.length;
    const withoutAvg = withoutContextData.reduce((sum, val) => sum + val, 0) / withoutContextData.length;
    const difference = withAvg - withoutAvg;
    
    // Only report significant differences (>10%)
    const significanceThreshold = Math.abs(withoutAvg * 0.1);
    
    if (Math.abs(difference) > significanceThreshold) {
      let impact;
      // Interpret the impact based on the metric
      switch(metric) {
        case 'steps':
          impact = difference > 0 ? 'more' : 'fewer';
          break;
        case 'sleepHours':
          impact = difference > 0 ? 'more' : 'less';
          break;
        case 'waterIntake':
          impact = difference > 0 ? 'more' : 'less';
          break;
        case 'mood':
          impact = difference > 0 ? 'better' : 'worse';
          break;
        default:
          impact = difference > 0 ? 'higher' : 'lower';
      }
      ...
      insights.push({
        ...
        impact,
        confidence: Math.min(0.9, (withContextData.length + withoutContextData.length) / 20)
      });
    }
  }
});
